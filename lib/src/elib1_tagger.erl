...
%% I use epp_dodger to paser the file and the new improved erl_scan
...
-import(lists, [foldl/3, member/2, partition/2,
		reverse/1, reverse/2]).

-export([all/0, test2/0, test3/0, erl2html/1, is_keyword/1]).

test1() -> erl2html0("./elib1_tagger2.erl").

test2() -> erl2chunks("./elib1_tagger2.erl").
...
my_parse(Toks) ->
    Toks1 = remove_junk(Toks),
    case erl_parse:parse_form(Toks1) of
	{error, X} ->
	    io:format("error in line:~p ~n", [X]);
	Ok ->
	    Ok
    end.
...
erl2chunks(File) ->
    {taggedBlocks, Chunks} = splitErlang(File),
    Tagged = collect_by_tag(Chunks),
    E = post_process(Tagged, #mod{}),
    elib1_misc:dump("chunks", E),
    true.

%% to do
%%   parse compile export import module
%%         ...
%%   the rest are kept in source form


post_process([{func,L}|T], #mod{funcs=F0}=E) ->
    post_process(T, E#mod{funcs = L ++ F0});
post_process([{func1,L}|T], #mod{funcs=F0}=E) ->
    L1 = [{{func,F,A},B} || {{func1,F,A},B} <- L],
    post_process(T, E#mod{funcs = L1 ++ F0});
post_process([{specification,L}|T], #mod{specs=F0}=E) ->
    post_process(T, E#mod{specs = L ++ F0});
post_process([{compile,L}|T], E) ->
    post_process(T, E#mod{compile = L});
post_process([{module,L}|T], E) ->
    post_process(T, E#mod{module = L});
post_process([{import,L}|T], E) ->
    post_process(T, E#mod{import = L});
post_process([{export,L}|T], E) ->
    post_process(T, E#mod{export = L});
post_process([{record,L}|T], E) ->
    post_process(T, E#mod{record = L});
post_process([H|T], E) ->
    io:format("chucking:~p~n",[element(1,H)]),
    post_process(T, E);
post_process([], E) ->
    E.

collect_by_tag(L) ->
    foldl(fun update/2, [], L).

update({K,_}=X, L) -> update(key(K), X, L).

key(X) when is_tuple(X) -> element(1, X);
key(X) when is_atom(X)  -> X.
...
splitErlang(File) ->
    %% io:format("converting:~s~n",[File]),
    {ok, Forms} = dodge_file(File),
    %% elib1_misc:dump("forms", Forms),
    {Anchors, Patches} = analyse(Forms),
    Raw = read_raw_forms(File),
    %% elib1_misc:dump("rawanchors", {Anchors, Raw}),
    Raw1 = merge_anchors(Anchors, Raw),
    %% elib1_misc:dump("raw1", Raw1),
    Raw2 = merge_forms(Raw1, Patches, []),
    Rtf = [final(I) || I <- Raw2],
    %% elib1_misc:dump("rtf", Rtf),
    {taggedBlocks, Rtf}.

erl2html(In, Out) ->
    Root = filename:basename(filename:rootname(In)),
    Tb = splitErlang(In),
    Html = to_html(Tb),
    file:write_file(Out, [prelude(Html)]),
...
merge_forms([{Tag,L1}|T], Patches, L) ->
    %% io:format("apply patches Toks=~p Patches=~p~n",[L1,Patches]),
    {L2, Patches1} = apply_patches(L1, Patches),
    %% io:format("Result =~p patches1=~p~n",[L2, Patches1]),
...
reformat(Tag, Ln, Val) ->
    io:format("reformat***:~p ~p ~p~n",[Tag,Ln,Val]),
    {Tag, Ln, Val}.
...
anchor1({specification, F, A}) ->
    ["<a name='",linkname(F,A),"'></a>"];
anchor1(_X) ->
    %% io:format("unknown anchor:~p~n",[_X]),
    "".
...
span(C, S) ->
    ["<span class=\"", C, "\">", htmlquote(S), "</span>"].

htmlquote("<" ++ T) -> ["&lt;"|htmlquote(T)];
htmlquote([H|T]) -> [H|htmlquote(T)];
htmlquote([]) -> [].

to_html1({white_space,X}) -> X;
to_html1({comment, X}) -> span("comment", X);
to_html1({var,V})      -> span("var", V);
to_html1({terminal,V})    -> V;
to_html1({string,V})    -> span("string", V);
to_html1({char,V})    -> V;
to_html1({integer,V})   -> span("int", V);
to_html1({float,V})   -> V;
to_html1({atom, V})     -> span("atom", V);
to_html1({anchor,F,A})  ->  ["<a name='",linkname(F,A),"'></a>"];
...
form() ->
    "$a + 97 + 16#23 + X + \"abc\023de\" 123. %% abc".


test7() ->
    erl_scan:string(form(), {1,1}, [text]).

test3() ->
    {done, {ok, Toks, _}, _} = erl_scan:tokens([], form(), {1,1}, [text,return]),
    Toks1 = remove_junk(Toks),
    io:format("Toks1=~p~n",[Toks1]),
    erl_parse:parse_form(Toks1).

remove_junk([{white_space,_,_}|T]) -> remove_junk(T);
remove_junk([{comment, _,_}|T]) -> remove_junk(T);
...
analyse(Forms) ->
    ...
    %% elib1_misc:dump("anchors", Anchors),
    %% elib1_misc:dump("calls", Calls),
    Imports = [{{F,A},Mod} ||
		  {attribute,_,import,{Mod,L}} <- Forms, {F,A} <- L],
    D = dict:from_list(Imports),
    %% io:format("compute patches Imports=~p Calls=~p~n",
    %% 	      [Imports,Calls]),
    Patches = [{Loc, resolve(X, D)} || {Loc, X} <- Calls],
    %% io:format("Patches=~p~n",[Patches]),
    %% elib1_misc:dump("patches", Patches),
    {Anchors, Patches}.
...
prelude(L) ->
    ["<html>
<head>
<link rel='stylesheet' href='./erl.css' type='text/css'>
</head>
<body>
<ul><pre>\n",L,"\n</pre></ul></body>"].
