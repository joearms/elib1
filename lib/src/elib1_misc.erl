...
%% Time-stamp: <2009-12-09 17:41:07 joe>
...
%% them? Answer: elib1_misc. elib1_misc is my dumping ground for a
...
added_files(Dir, Fun) ->
    ...
    Result = Fun(),
    After =  elib1_find:files(Dir, "*", true),
    Added = filter(fun(I) ->
			   not member(I, Before)
		   end, After),
    {Result, Added}.
...
extract_attribute_test() ->
    {ok, _} = extract_attribute(elib1_misc,vsn).
...
file2numberedlines_test() ->
    %% test this file
    [{1,"-module(elib1_misc).\n"},
     {2,"%% elib1_misc   Miscellaneous functions\n"}|_] =
	file2numberedlines("./elib1_misc.erl").
...
file2md5(File) ->
	...
	{ok, P} -> file2md5_loop(P, erlang:md5_init());
	Error   -> Error
    end.
...
file2md5_loop(P, C) ->
	...
	{ok, Bin} ->
	    file2md5_loop(P, erlang:md5_update(C, Bin));
	eof ->
	    file:close(P),
...
-spec include_file(FileName::string()) ->
...
%%----------------------------------------------------------------------
%% @doc Interlieve a symbol Sep between elements in a list.
%% Given a list [X1,X2,...,Xn] makes the list.
%% [X1,Sep,X2,Sep,...,Sep,Xn].

-spec interlieve(Sep::X, In::[X]) -> Out::[X].

interlieve(_, [X]) -> [X];
interlieve(_, []) -> [];
interlieve(Sep, [H|T]) -> [H,Sep|interlieve(Sep, T)].
...
-spec check_io_list(iolist()) -> boolean().

check_io_list([H|T]) ->
    check_io_list(H) and check_io_list(T);
...
%% This is evaluated for its side effect. Now well tested at all.
...
read_at_most_n_lines_test() ->
    ["-module(elib1_misc).\n",
     "%% elib1_misc   Miscellaneous functions\n"]
	= read_at_most_n_lines("./elib1_misc.erl", 2).
...
string2toks_test() ->
    [{atom,"abc"},
     ...
     {int,12},
     {punct,"."},
     {int,23}] = elib1_misc:string2toks("abc,123'abc'{,12.23").
...
collect_string_test() ->
    {"abc"," def"} = elib1_misc:collect_string("\"abc\" def"),
    {"abc"," def"} = elib1_misc:collect_string("\'abc\' def").
...
%% ["who",<<" is feeling ">>,"how"]
%% 3> elib1_misc:expand_template(Template,[{"who","Sue"},{"how","happy"}]).
