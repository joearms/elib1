...
expand_file(File, Bs) ->
    Bindings = make_bindings(Bs, erl_eval:new_bindings()),
    {ok, B} = file:read_file(File),
    expand_string(binary_to_list(B), Bindings).

make_bindings([{K,V}|T], B) ->
    make_bindings(T, erl_eval:add_binding(K,V,B));
make_bindings([], B) ->
    B.
